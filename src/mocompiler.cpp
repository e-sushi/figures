...
#define ErrorMessage(...) LogE("MathObjectCompiler", compiler.file->name, ":", compiler.curt->line, ":", compiler.curt->column, ": ", __VA_ARGS__)
...
b32 compile_parse_instructions(){
    ...
    while(1){
        ...
        if(compiler.curt->type == Token_CloseBracket){
			break;
		}
    }
        ...
        switch(*compiler.stream.str){
            ...
            default:{
                ...
                if(is_digit(*start.str)){
                    is_word = false;
                    while(is_digit(*compiler.stream.str)){
						advance_stream();
					}
                }else while(compiler.stream){
                    if(is_whitespace(*compiler.stream.str) || match_any(*compiler.stream.str, ':', '{', '}', '(', ')', ';', '[', ']', '"', ',', '.')){
                        break;
                    }
                    advance_stream();
                }
